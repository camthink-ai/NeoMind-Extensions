...
use neomind_extension_sensevoice_asr::SenseVoiceAsrExtension;

/// Metadata consistency: id and version match what metadata.json declares.
#[tokio::test]
async fn protocol_metadata() {
    let kit = TestKit::new(SenseVoiceAsrExtension::new());
...
async fn protocol_unknown_command() {
    let mut kit = TestKit::new(SenseVoiceAsrExtension::new());
    kit.start().await;
