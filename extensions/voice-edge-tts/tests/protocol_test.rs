...
use neomind_extension_voice_edge_tts::VoiceEdgeTtsExtension;

/// Metadata consistency: id and version match what metadata.json declares.
#[tokio::test]
async fn protocol_metadata() {
    let kit = TestKit::new(VoiceEdgeTtsExtension::new());
...
async fn protocol_unknown_command() {
    let mut kit = TestKit::new(VoiceEdgeTtsExtension::new());
    kit.start().await;
