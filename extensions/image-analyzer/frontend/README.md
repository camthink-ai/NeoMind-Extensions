...
- `dist/image-analyzer-components.umd.js` - UMD bundle for dashboard
...
**Props:**
- `title` - Component title (default: "Image Analyzer")
...
```tsx
import { ImageAnalyzer } from '@neomind/image-analyzer-frontend'

<ImageAnalyzer
  dataSource={{ extensionId: 'image-analyzer' }}
...
```typescript
const metrics = await getExtensionMetrics('image-analyzer')
