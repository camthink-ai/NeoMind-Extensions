...
use neomind_extension_stream_player::StreamPlayerExtension;

/// Metadata consistency: id and version match what metadata.json declares.
#[tokio::test]
async fn protocol_metadata() {
    let kit = TestKit::new(StreamPlayerExtension::new());
...
async fn protocol_unknown_command() {
    let mut kit = TestKit::new(StreamPlayerExtension::new());
    kit.start().await;
...
#[tokio::test]
async fn protocol_get_player_info() {
    let mut kit = TestKit::new(StreamPlayerExtension::new());
    kit.start().await;
    // Execute through the IPC path — timing assertion catches deadlocks
    let result = kit.execute_command("get_player_info", &json!({})).await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "unexpected error for get_player_info: {msg}"
            );
        }
    }
}
#[tokio::test]
async fn protocol_list_sources() {
    let mut kit = TestKit::new(StreamPlayerExtension::new());
    kit.start().await;
    // Execute through the IPC path — timing assertion catches deadlocks
    let result = kit.execute_command("list_sources", &json!({})).await;
    // We verify the command doesn't deadlock/panic; specific result
    // depends on runtime state (models, connections, etc.)
    match result {
        Ok(_) => { /* command succeeded */ }
        Err(e) => {
            ...
            assert!(
                ...
                || msg.contains("error") || msg.contains("service")
                || msg.contains("timeout") || msg.contains("model"),
                "unexpected error for list_sources: {msg}"
            );
        }
    }
}
