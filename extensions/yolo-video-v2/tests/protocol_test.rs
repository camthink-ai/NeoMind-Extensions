...
use neomind_extension_yolo_video_v2::YoloVideoProcessorV2;

/// Metadata consistency: id and version match what metadata.json declares.
#[tokio::test]
async fn protocol_metadata() {
    let kit = TestKit::new(YoloVideoProcessorV2::new());
...
async fn protocol_unknown_command() {
    let mut kit = TestKit::new(YoloVideoProcessorV2::new());
    kit.start().await;
...
#[tokio::test]
async fn protocol_get_stream_stats() {
    let mut kit = TestKit::new(YoloVideoProcessorV2::new());
    kit.start().await;
    // Execute through the IPC path — timing assertion catches deadlocks
    let result = kit.execute_command("get_stream_stats", &json!({})).await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "unexpected error for get_stream_stats: {msg}"
            );
        }
    }
}
