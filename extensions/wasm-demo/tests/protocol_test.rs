...
async fn protocol_metadata() {
    let kit = TestKit::new(WasmDemoExtension::new());
...
async fn protocol_unknown_command() {
    let mut kit = TestKit::new(WasmDemoExtension::new());
    kit.start().await;
...
#[tokio::test]
async fn protocol_get_value() {
    let mut kit = TestKit::new(WasmDemoExtension::new());
    kit.start().await;
    // Execute through the IPC path — timing assertion catches deadlocks
    let result = kit.execute_command("get_value", &json!({})).await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "unexpected error for get_value: {msg}"
            );
        }
    }
}
#[tokio::test]
async fn protocol_increment() {
    let mut kit = TestKit::new(WasmDemoExtension::new());
    kit.start().await;
    // Execute through the IPC path — timing assertion catches deadlocks
    let result = kit.execute_command("increment", &json!({"amount": 5})).await;
    // We verify the command doesn't deadlock/panic; specific result
    // depends on runtime state (models, connections, etc.)
    match result {
        Ok(_) => { /* command succeeded */ }
        Err(e) => {
            ...
            assert!(
                ...
                || msg.contains("error") || msg.contains("service")
                || msg.contains("timeout") || msg.contains("model"),
                "unexpected error for increment: {msg}"
            );
        }
    }
}
