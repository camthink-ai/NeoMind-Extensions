...
mod tests {
    ...
    use neomind_extension_weather_forecast::WeatherExtension;
    ...
    fn test_frontend_assets_consistency() {
        ...
        for (ext_id, expected_component) in extensions_with_frontend {
            let metadata: Value = match ext_id {
                "weather-forecast" => serde_json::from_str(include_str!("../metadata.json")).unwrap(),
                "image-analyzer" => serde_json::from_str(include_str!("../../image-analyzer/metadata.json")).unwrap(),
                "yolo-video" => serde_json::from_str(include_str!("../../yolo-video/metadata.json")).unwrap(),
            ...
            let frontend_json: Value = match ext_id {
                "weather-forecast" => serde_json::from_str(include_str!("../frontend/frontend.json")).unwrap(),
                "image-analyzer" => serde_json::from_str(include_str!("../../image-analyzer/frontend/frontend.json")).unwrap(),
                "yolo-video" => serde_json::from_str(include_str!("../../yolo-video/frontend/frontend.json")).unwrap(),
    ...
    fn test_metadata_basic() {
        ...
        assert_eq!(meta.id, "weather-forecast");
        assert_eq!(meta.name, "Weather Forecast");
