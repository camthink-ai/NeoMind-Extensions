...
async fn protocol_metadata() {
    let kit = TestKit::new(WeatherExtension::new());
...
async fn protocol_unknown_command() {
    let mut kit = TestKit::new(WeatherExtension::new());
    kit.start().await;
...
#[tokio::test]
async fn protocol_get_weather() {
    let mut kit = TestKit::new(WeatherExtension::new());
    kit.start().await;
    // Execute through the IPC path — timing assertion catches deadlocks
    let result = kit.execute_command("get_weather", &json!({})).await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "unexpected error for get_weather: {msg}"
            );
        }
    }
}
