...
use neomind_extension_voice_assistant::VoiceAssistantExtension;

/// Metadata consistency: id and version match what metadata.json declares.
#[tokio::test]
async fn protocol_metadata() {
    let kit = TestKit::new(VoiceAssistantExtension::new());
...
async fn protocol_unknown_command() {
    let mut kit = TestKit::new(VoiceAssistantExtension::new());
    kit.start().await;
