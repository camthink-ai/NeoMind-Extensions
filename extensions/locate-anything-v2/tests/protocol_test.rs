...
use neomind_extension_locate_anything_v2::LocateAnythingExtension;

/// Metadata consistency: id and version match what metadata.json declares.
#[tokio::test]
async fn protocol_metadata() {
    let kit = TestKit::new(LocateAnythingExtension::new());
...
async fn protocol_unknown_command() {
    let mut kit = TestKit::new(LocateAnythingExtension::new());
    kit.start().await;
...
#[tokio::test]
async fn protocol_check_status() {
    let mut kit = TestKit::new(LocateAnythingExtension::new());
    kit.start().await;
    // Execute through the IPC path — timing assertion catches deadlocks
    let result = kit.execute_command("check_status", &json!({})).await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "unexpected error for check_status: {msg}"
            );
        }
    }
}
#[tokio::test]
async fn protocol_detect() {
    let mut kit = TestKit::new(LocateAnythingExtension::new());
    kit.start().await;
    // Execute through the IPC path — timing assertion catches deadlocks
    let result = kit.execute_command("detect", &json!({"image": "invalid"})).await;
    // We verify the command doesn't deadlock/panic; specific result
    // depends on runtime state (models, connections, etc.)
    match result {
        Ok(_) => { /* command succeeded */ }
        Err(e) => {
            ...
            assert!(
                ...
                || msg.contains("error") || msg.contains("service")
                || msg.contains("timeout") || msg.contains("model"),
                "unexpected error for detect: {msg}"
            );
        }
    }
}
