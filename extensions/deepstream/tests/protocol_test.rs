...
async fn protocol_metadata() {
    let kit = TestKit::new(DeepStreamExtension::new());
...
async fn protocol_unknown_command() {
    let mut kit = TestKit::new(DeepStreamExtension::new());
    kit.start().await;
...
#[tokio::test]
async fn protocol_system_status() {
    let mut kit = TestKit::new(DeepStreamExtension::new());
    kit.start().await;
    // Execute through the IPC path — timing assertion catches deadlocks
    let result = kit.execute_command("system_status", &json!({})).await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "unexpected error for system_status: {msg}"
            );
        }
    }
}
#[tokio::test]
async fn protocol_list_streams() {
    let mut kit = TestKit::new(DeepStreamExtension::new());
    kit.start().await;
    // Execute through the IPC path — timing assertion catches deadlocks
    let result = kit.execute_command("list_streams", &json!({})).await;
    // We verify the command doesn't deadlock/panic; specific result
    // depends on runtime state (models, connections, etc.)
    match result {
        Ok(_) => { /* command succeeded */ }
        Err(e) => {
            ...
            assert!(
                ...
                || msg.contains("error") || msg.contains("service")
                || msg.contains("timeout") || msg.contains("model"),
                "unexpected error for list_streams: {msg}"
            );
        }
    }
}
